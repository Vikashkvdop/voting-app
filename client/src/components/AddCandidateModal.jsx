...
const AddCandidateModal = () => {
    ...
    const electionId=useSelector(state=>state?.vote?.addCandiateElectionId)
    ...
    const addCandidate=async(e)=>{
       try {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL}/candidates`, candidateInfo, {withCredentials:true, headers:{Authorization: `Bearer ${token}`}})
        navigate(0)
       } catch (error) {
        console.log(error)
       }
